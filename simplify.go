...
// license that can be found in the third_party/go/LICENSE file.
		...
		if eltType != nil {
			for i, x := range outer.Elts {
				...
				if t, ok := x.(*ast.KeyValueExpr); ok {
					if keyType != nil {
						s.simplifyLiteral(keyType, t.Key, &t.Key)
					}
					x = t.Value
					px = &t.Value
				}
				s.simplifyLiteral(eltType, x, px)
			}
...
func (s simplifier) simplifyLiteral(astType, x ast.Expr, px *ast.Expr) {
	...
	if inner, ok := x.(*ast.CompositeLit); ok {
		if equalExpr(astType, inner.Type) {
			inner.Type = nil
		}
	}
